...
def main():
    ...
    if not should_resume:
        ...
        # Branch Management Suggestion
        if is_git_repo():
            current_branch = get_current_branch()
            if current_branch == "main" or current_branch == "master":
                console.print()
                print_info(f"Current branch is '{current_branch}'.")
                if ask_confirm("Would you like to create a feature branch for this project?"):
                    branch_name = ask_input("Enter branch name (e.g. feature-project-name)").strip().lower().replace(" ", "-")
                    if branch_name:
                        success, message = create_branch(branch_name)
                        if success:
                            print_success(message)
                        else:
                            print_error(f"Failed to create branch: {message}")
    ...
    while STATE["current_phase"] < len(STATE["phases"]):
        ...
        if create_issues and not existing_tasks:
            ...
            # Create Milestone for the phase (optional but good)
            milestone_number = None
            m_success, m_data = create_milestone(STATE["repo_url"], f"Phase {phase_number}: {phase_name}", f"Tasks related to Phase {phase_number}")
            if m_success:
                milestone_number = m_data
                print_success(f"GitHub Milestone created: {phase_name}")

            # Ensure Phase Label exists
            phase_label = f"phase-{phase_number}"
            create_label(STATE["repo_url"], phase_label, "cfd3d7", f"Tasks for {phase_label}")

            for task_obj in tasks:
                task_text = task_obj["task"] if isinstance(task_obj, dict) else task_obj
                body = f"Task generated by execution_orecal for Phase {phase_number}.\n\nPhase: {phase_name}"
                labels = [phase_label, "todo"]
                
                success, url_or_msg = create_issue(
                    STATE["repo_url"], 
                    title=f"[{phase_label}] {task_text}", 
                    body=body,
                ...
                if success:
                    print_success(f"Issue created: {url_or_msg}")
                else:
                    print_error(f"Failed to create issue: {url_or_msg}")
            console.print()
        ...
        if success:
            ...
            if is_git_repo():
                console.print()
                if ask_confirm(f"Phase {phase_number} complete! Would you like to create a Pull Request?", default=True):
                    current_branch = get_current_branch()
                    pr_title = f"Phase {phase_number}: {phase_name}"
                    pr_body = f"Completed all tasks for Phase {phase_number}.\n\nTasks:\n"
                    tasks = get_tasks(idx)
                    for t in tasks:
                        status = "✅" if t.get("completed") else "☐"
                        pr_body += f"- {status} {t['task']}\n"
                    
                    pr_success, pr_url = create_pull_request(STATE["repo_url"], pr_title, current_branch, body=pr_body)
                    if pr_success:
                        print_success(f"Pull Request created: {pr_url}")
                    else:
                        print_error(f"Failed to create PR: {pr_url}")
                        print_note("Note", ["Make sure you have pushed your changes to GitHub before creating a PR."])
                console.print()
        else:
